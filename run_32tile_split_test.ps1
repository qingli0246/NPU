# NPU 32-Tile SPLIT模式全量并行计算测试脚本
# 功能：编译并运行 32-Tile SPLIT模式测试，验证所有 Tile 的并行计算能力
# 输出：仿真结果将保存到 TB/npu_32tile_split_test.log 文件
...
Write-Host "  NPU 32-Tile SPLIT模式全量测试" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$TestFile = Join-Path $TBDir "tb_npu_32tile_split_test.v"
$OutputExe = Join-Path $ProjectRoot "tb_npu_32tile_split_test.exe"
$WaveFile = Join-Path $ProjectRoot "TB\npu_32tile_split_test.vcd"
$LogFile = Join-Path $ProjectRoot "TB\npu_32tile_split_test.log"
...
switch ($Tool.ToLower()) {
    "iverilog" {
        ...
        $CompileCmd = "iverilog -g2012 -o `"$OutputExe`" -I `"$RTLDir`" `"$(Join-Path $TBDir 'tb_npu_32tile_split_test.v')`" `"$(Join-Path $RTLDir 'npu_top.v')`" `"$(Join-Path $RTLDir 'npu_ctrl.v')`" `"$(Join-Path $RTLDir 'npu_compute_pool.v')`" `"$(Join-Path $RTLDir 'npu_tile.v')`" `"$(Join-Path $RTLDir 'npu_dma_rd.v')`" `"$(Join-Path $RTLDir 'npu_dma_wr.v')`" `"$(Join-Path $RTLDir 'npu_weight_cache.v')`" `"$(Join-Path $RTLDir 'npu_axi4_bridge.v')`""
...
Write-Host "  gtkwave TB\npu_32tile_split_test.vcd" -ForegroundColor Yellow
Write-Host ""
