# ============================================================
#  NPU AXI Slave模块测试脚本
#  测试 npu_axi_slave 模块（联合npu_config）并保存结果到日志文件
# ============================================================
...
$TestFile = Join-Path $ScriptDir "tb_npu_axi_slave_test.v"
$OutputDir = Join-Path $ScriptDir "npu_axi_slave_test_output"
...
Write-Host "  NPU AXI Slave模块测试运行器" -ForegroundColor Cyan
...
$compile_args = @(
    "-o", (Join-Path $OutputDir "tb_npu_axi_slave"),
    "-I", $RTL_Dir,
    $TestFile,
    (Join-Path $RTL_Dir "npu_axi_slave.v"),
    (Join-Path $RTL_Dir "npu_config.v"),
    (Join-Path $RTL_Dir "npu_buffer_mgr.v")
)
...
$simulation_exe = Join-Path $OutputDir "tb_npu_axi_slave"

# 保存当前目录，然后切换到TB目录（确保VCD文件生成在正确位置）
...
# 移动VCD波形文件到输出目录
$VcdFile = Join-Path $ScriptDir "npu_axi_slave_test.vcd"
if (Test-Path $VcdFile) {
    Move-Item -Path $VcdFile -Destination $OutputDir -Force
    Write-Host "[信息] 波形文件已保存到: $(Join-Path $OutputDir 'npu_axi_slave_test.vcd')" -ForegroundColor Green
}
