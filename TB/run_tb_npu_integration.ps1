...
# 配置路径
$TEST_FILE = "F:\complication-6\NPU\TB\tb_npu_integration_test.v"
$OUTPUT_DIR = "F:\complication-6\NPU\TB\npu_integration_test_output"
$RTL_DIR = "F:\complication-6\NPU\rtl"

Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  NPU 联合测试运行器" -ForegroundColor Cyan
...
$compile_args = @(
    "-g2012",
    "-o", "$OUTPUT_DIR\tb_npu_integration.exe",
    "-I", "$RTL_DIR",
    "$TEST_FILE",
    "$RTL_DIR\npu_axi_slave.v",
    "$RTL_DIR\npu_config.v",
...
try {
    ...
    # 执行仿真
    $process = Start-Process -FilePath "vvp" -ArgumentList "`"$OUTPUT_DIR\tb_npu_integration.exe`"" -RedirectStandardOutput "$OUTPUT_DIR\test_log.txt" -PassThru -NoNewWindow -Wait
    ...
    # 移动波形文件到输出目录
    $vcd_file = "npu_integration_test.vcd"
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  联合测试运行结束" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan